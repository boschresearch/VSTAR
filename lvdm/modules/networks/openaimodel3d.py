...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None, batch_size=None, time_index=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                ...
                x = layer(x, context, time_index=time_index)
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, context=None, features_adapter=None, fps=16, **kwargs):
        ...
        time_index = kwargs.get("step_index", None)
        if self.fps_cond:
            if type(fps) == int:
        ...
        if context.shape[0] != t:
            context = context.repeat_interleave(repeats=t, dim=0) # (1,77,1024) -> (16,77,1024)
        emb = emb.repeat_interleave(repeats=t, dim=0) # (1,1280) -> (16,1280)
        ...
        for id, module in enumerate(self.input_blocks):
            h = module(h, emb, context=context, batch_size=b,time_index=time_index)
            if id ==0 and self.addition_attention:
                h = self.init_attn(h, emb, context=context, batch_size=b, time_index=time_index)
        ...
        h = self.middle_block(h, emb, context=context, batch_size=b, time_index=time_index)
