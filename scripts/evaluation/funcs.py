...
def batch_ddim_sampling(model, cond, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1.0, \
                        cfg_scale=1.0, temporal_cfg_scale=None, verbose=False, x_T=None, **kwargs):
    ...
    if cfg_scale != 1.0:
        if uncond_type == "empty_seq":
            ...
            uc_emb = model.get_learned_conditioning(prompts) # (1,77,1024)
...
def save_videos(batch_tensors, savedir, filenames, fps=10,ext_name="mp4"):
    # b,samples,c,t,h,w
    ...
    for idx, vid_tensor in enumerate(batch_tensors):
        ...
        savepath = os.path.join(savedir, f"{filenames[idx]}.{ext_name}")
        if ext_name == "mp4":
